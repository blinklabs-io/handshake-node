...
All notifications provided by handshake-node require registration to opt-in.  For example,
...
detected by type asserting the error in a *hnsjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

	  amount, err := client.GetBalance("")
	  if err != nil {
	  	if jerr, ok := err.(*hnsjson.RPCError); ok {
	  		switch jerr.Code {
	  		case hnsjson.ErrRPCUnimplemented:
