...
// oldHnsHomeDir returns the OS specific home directory handshake-node used
// prior to version 0.3.3.  This has since been replaced with
// hnsutil.AppDataDir, but this function is still provided for the automatic
// upgrade path.
func oldHnsHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "handshake-node")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".handshake-node")
	}
...
// location prior to version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to
...
// upgradeDataPaths moves the application data from its location prior to
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		hnsLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old config into new location if needed.
		...
		if ohpEmpty {
			...
			hnsLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to handshake-node as new versions require it.
