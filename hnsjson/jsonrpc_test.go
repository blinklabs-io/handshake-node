...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if hnsjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *hnsjson.RPCError {
				return hnsjson.NewRPCError(hnsjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := hnsjson.MarshalResponse(hnsjson.RpcVersion1, testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// supported.
	wantErr := hnsjson.Error{ErrorCode: hnsjson.ErrInvalidType}
	_, err = hnsjson.MarshalResponse(hnsjson.RpcVersion1, make(chan int), nil, nil)
	if jerr, ok := err.(hnsjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = hnsjson.MarshalResponse(hnsjson.RpcVersion1, 1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			hnsjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			hnsjson.ErrRPCMethodNotFound,
