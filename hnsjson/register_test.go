...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   hnsjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{hnsjson.UFWalletOnly, "UFWalletOnly"},
		{hnsjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{hnsjson.UFNotification, "UFNotification"},
		{hnsjson.UFWalletOnly | hnsjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{hnsjson.UFWalletOnly | hnsjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := hnsjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err:   hnsjson.Error{ErrorCode: hnsjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: hnsjson.Error{ErrorCode: hnsjson.ErrEmbeddedType},
		},
		{
			...
			err: hnsjson.Error{ErrorCode: hnsjson.ErrUnexportedField},
		},
		{
		...
		},
		{
			...
			err: hnsjson.Error{ErrorCode: hnsjson.ErrNonOptionalField},
		},
		{
			...
			err: hnsjson.Error{ErrorCode: hnsjson.ErrNonOptionalDefault},
		},
		{
			...
			err: hnsjson.Error{ErrorCode: hnsjson.ErrMismatchedDefault},
		},
	}
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	hnsjson.MustRegisterCmd("panicme", 0, 0)
}
...
func TestRegisteredCmdMethods(t *testing.T) {
	...
	methods := hnsjson.RegisteredCmdMethods()
	if len(methods) == 0 {
