...
func TestMarshalCmd(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		bytes, err := hnsjson.MarshalCmd(hnsjson.RpcVersion1, test.id, test.cmd)
		if err != nil {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v)",
				i, test.name, err, err)
			continue
...
func TestMarshalCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			id:   1,
			cmd:  (*hnsjson.GetBlockCmd)(nil),
			err:  hnsjson.Error{ErrorCode: hnsjson.ErrInvalidType},
		},
		{
			name: "nil instance of registered type",
			id:   []int{0, 1},
			cmd:  &hnsjson.GetBlockCountCmd{},
			err:  hnsjson.Error{ErrorCode: hnsjson.ErrInvalidType},
		},
	}
