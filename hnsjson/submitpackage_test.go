...
func TestSubmitPackageCmd(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			unmarshalled: &hnsjson.JsonSubmitPackageCmd{
		...
		},
		{
			...
			unmarshalled: &hnsjson.JsonSubmitPackageCmd{
				RawTxs:        []string{"hex1", "hex2"},
				MaxFeeRate:    hnsjson.Float64(0.1),
				MaxBurnAmount: nil,
			},
		},
		{
			...
			unmarshalled: &hnsjson.JsonSubmitPackageCmd{
				...
				MaxFeeRate:    hnsjson.Float64(0.25),
				MaxBurnAmount: hnsjson.Float64(0.001),
			},
		},
		{
		...
		},
		{
			...
			unmarshalled: &hnsjson.JsonSubmitPackageCmd{
				RawTxs:        []string{"hex1", "hex2"},
				MaxFeeRate:    hnsjson.Float64(0.0),
				MaxBurnAmount: nil,
			},
		},
	}
	...
	for i, test := range tests {
		...
		// creation function.
		marshalled, err := hnsjson.MarshalCmd(
			hnsjson.RpcVersion1, testID, test.staticCmd(),
		)
		require.NoError(
		...
		// creation function.
		marshalled, err = hnsjson.MarshalCmd(
			hnsjson.RpcVersion1, testID, cmd,
		)
		require.NoError(
			t, err, "MarshalCmd #%d (%s) unexpected in test", i,
			test.name,
...
func TestSubmitPackageResultUnmarshalJSON(t *testing.T) {
	...
	tests := []struct {
		name     string
		json     string
		expected hnsjson.SubmitPackageResult
		wantErr  bool
	}{
		{
			...
			expected: hnsjson.SubmitPackageResult{
				PackageMsg: "success",
				TxResults: map[string]hnsjson.SubmitPackageTxResult{
					"wtxid1": {
		...
		},
		{
			...
			expected: hnsjson.SubmitPackageResult{
				...
				TxResults: map[string]hnsjson.SubmitPackageTxResult{
					"wtxid1": {
						TxID:  *mustParseHash("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"),
						Error: hnsjson.String("insufficient fee"),
					},
				},
			},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var result hnsjson.SubmitPackageResult
