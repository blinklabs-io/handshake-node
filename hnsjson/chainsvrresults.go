...
type GetTxOutSetInfoResult struct {
	...
	TotalAmount    hnsutil.Amount `json:"total_amount"`
}
...
func (g *GetTxOutSetInfoResult) UnmarshalJSON(data []byte) error {
	...
	amount, err := hnsutil.NewAmount(aux.TotalAmount)
	if err != nil {
		return err
	}
...
// field since it requires wallet access, which is outside the scope of handshake-node.
...
func (f *FundRawTransactionResult) UnmarshalJSON(data []byte) error {
	...
	fee, err := hnsutil.NewAmount(rawRes.Fee)
	if err != nil {
		return err
	}
