...
func TestHelpReflectInternals(t *testing.T) {
	...
	for i, test := range tests {
		...
		key := hnsjson.TstReflectTypeToJSONType(xT, test.reflectType)
		if key != test.key {
		...
		examples, isComplex := hnsjson.TstReflectTypeToJSONExample(xT,
		...
		helpText := hnsjson.TstResultTypeHelp(xT, test.reflectType, "fdk")
		...
		isValid := hnsjson.TstIsValidResultType(test.reflectType.Kind())
...
func TestResultStructHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		results := hnsjson.TstResultStructHelp(xT, test.reflectType, 0)
...
func TestHelpArgInternals(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := hnsjson.TstArgHelp(xT, test.reflectType, test.defaults,
			test.method)
		if help != test.help {
...
func TestMethodHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := hnsjson.TestMethodHelp(xT, test.reflectType,
...
func TestGenerateHelpErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			method:      "help",
			resultTypes: []any{(*complex64)(nil)},
			err:         hnsjson.Error{ErrorCode: hnsjson.ErrInvalidType},
		},
		{
			name:        "missing description",
			method:      "help",
			resultTypes: []any{(*string)(nil), nil},
			err:         hnsjson.Error{ErrorCode: hnsjson.ErrMissingDescription},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := hnsjson.GenerateHelp(test.method, nil,
			test.resultTypes...)
...
func TestGenerateHelp(t *testing.T) {
	...
	help, err := hnsjson.GenerateHelp("help", descs)
	if err != nil {
