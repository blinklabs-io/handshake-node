...
func ExampleMarshalCmd() {
	...
	// optional fields.  Also, notice the call to hnsjson.Bool which is a
	// convenience function for creating a pointer out of a primitive for
	// optional parameters.
	blockHash := "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"
	gbCmd := hnsjson.NewGetBlockCmd(blockHash, hnsjson.Int(0))
	...
	id := 1
	marshalledBytes, err := hnsjson.MarshalCmd(hnsjson.RpcVersion1, id, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleUnmarshalCmd() {
	...
	gbCmd, ok := cmd.(*hnsjson.GetBlockCmd)
	if !ok {
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := hnsjson.MarshalResponse(hnsjson.RpcVersion1, 1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
