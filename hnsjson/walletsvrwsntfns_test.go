...
func TestWalletSvrWsNtfns(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			unmarshalled: &hnsjson.AccountBalanceNtfn{
				Account:   "acct",
				Balance:   1.25,
				Confirmed: true,
			},
		},
		{
			...
			unmarshalled: &hnsjson.BtcdConnectedNtfn{
				Connected: true,
			},
		},
		{
			...
			unmarshalled: &hnsjson.WalletLockStateNtfn{
				Locked: true,
			},
		},
		{
			...
			staticNtfn: func() interface{} {
				...
				return hnsjson.NewNewTxNtfn("acct", result)
			},
			...
			unmarshalled: &hnsjson.NewTxNtfn{
				Account: "acct",
				Details: hnsjson.ListTransactionsResult{
					Abandoned:         false,
					Account:           "acct",
					Address:           "1Address",
					BIP125Replaceable: "unknown",
