...
func TestChainSvrCustomResults(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: &hnsjson.GetZmqNotificationResult{{
				Type: "pubrawblock",
...
func TestGetTxOutSetInfoResult(t *testing.T) {
	...
	tests := []struct {
		name   string
		result string
		want   hnsjson.GetTxOutSetInfoResult
	}{
		{
			...
			want: hnsjson.GetTxOutSetInfoResult{
				...
				DiskSize: 1,
				TotalAmount: func() hnsutil.Amount {
					a, err := hnsutil.NewAmount(0.2)
					if err != nil {
						panic(err)
					}
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var out hnsjson.GetTxOutSetInfoResult
...
func TestChainSvrMiningInfoResults(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected hnsjson.GetMiningInfoResult
	}{
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var miningInfoResult hnsjson.GetMiningInfoResult
...
func TestGetNetworkInfoWarnings(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected hnsjson.GetNetworkInfoResult
	}{
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var infoResult hnsjson.GetNetworkInfoResult
