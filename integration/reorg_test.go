...
func TestReorgFromForkPoint(t *testing.T) {
	...
	longer, err := rpctest.New(&chaincfg.RegressionNetParams, nil, []string{}, "")
	require.NoError(t, err)
	require.NoError(t, longer.SetUp(false, 0))
	t.Cleanup(func() { require.NoError(t, longer.TearDown()) })

	shorter, err := rpctest.New(&chaincfg.RegressionNetParams, nil, []string{}, "")
	require.NoError(t, err)
	...
	var activeTip, forkTip *hnsjson.GetChainTipsResult
