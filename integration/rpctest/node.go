...
import (
	...
	rpc "github.com/blinklabs-io/handshake-node/rpcclient"
)

// nodeConfig contains all the args, and data required to launch a handshake-node process
...
// arguments returns an array of arguments that be used to launch the handshake-node
// process.
...
// command returns the exec.Cmd which will be used to start the handshake-node process.
...
// to the handshake-node process that is launched via Start().
...
// as the base for the log and data directories for handshake-node.
...
// start creates a new handshake-node process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.dataDir, "handshake-node.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running handshake-node process, and waits until it exits
...
// shutdown terminates the running handshake-node process, and cleans up all
