...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr hnsutil.Address) (*btcec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr hnsutil.Address) ([]byte, error) {
...
func signAndCheck(msg string, tx *wire.MsgTx, idx int, inputAmt int64, pkScript []byte,
	...
	sigScript, err := SignTxOutput(&chaincfg.RegressionNetParams, tx, idx,
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := hnsutil.NewAddressPubKeyHash(
				hnsutil.Hash160(pk), &chaincfg.RegressionNetParams)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			sigScript, err := SignTxOutput(&chaincfg.RegressionNetParams,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := key.PubKey().SerializeUncompressed()
			address, err := hnsutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := hnsutil.NewAddressScriptHash(
				pkScript, &chaincfg.RegressionNetParams)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := hnsutil.NewAddressPubKey(pk1,
				&chaincfg.RegressionNetParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := hnsutil.NewAddressPubKey(pk2,
				&chaincfg.RegressionNetParams)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*hnsutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
