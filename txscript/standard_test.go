...
// newAddressPubKey returns a new hnsutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) hnsutil.Address {
	addr, err := hnsutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new hnsutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) hnsutil.Address {
	addr, err := hnsutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new hnsutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) hnsutil.Address {
	addr, err := hnsutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// newAddressTaproot returns a new hnsutil.AddressTaproot from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressTaproot(scriptHash []byte) hnsutil.Address {
	addr, err := hnsutil.NewAddressTaproot(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
		panic("invalid script hash in test source")
	}
...
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			addrs: []hnsutil.Address{newAddressTaproot(
...
// bogusAddress implements the hnsutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the hnsutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := hnsutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := hnsutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := hnsutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := hnsutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := hnsutil.NewAddressPubKey(hexToBytes("0411"+
	...
	p2wsh, err := hnsutil.NewAddressWitnessScriptHash(hexToBytes("e981bd992a43650657"+
	...
	p2tr, err := hnsutil.NewAddressTaproot(hexToBytes("3a8e170b546c3b122ab9c175e"+
	...
	p2wpkh, err := hnsutil.NewAddressWitnessPubKeyHash(hexToBytes("748e50366adb8"+
	...
	tests := []struct {
		...
		{(*hnsutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*hnsutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*hnsutil.AddressPubKey)(nil), "", errUnsupportedAddress},
		{(*hnsutil.AddressWitnessPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*hnsutil.AddressWitnessScriptHash)(nil), "", errUnsupportedAddress},
		{(*hnsutil.AddressTaproot)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*hnsutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*hnsutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
