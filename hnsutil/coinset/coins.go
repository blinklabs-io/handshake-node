...
func (cs *CoinSet) TotalValue() (value hnsutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue hnsutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue hnsutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount hnsutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue hnsutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue hnsutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue hnsutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue hnsutil.Amount, coins []Coin) (Coins, error) {
...
// hnsutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() hnsutil.Amount {
