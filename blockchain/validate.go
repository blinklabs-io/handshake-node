...
func IsCoinBase(tx *hnsutil.Tx) bool {
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *hnsutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *hnsutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *hnsutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *hnsutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *hnsutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *hnsutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func CheckTransactionInputs(tx *hnsutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *hnsutil.Block) error {
	b.chainLock.Lock()
