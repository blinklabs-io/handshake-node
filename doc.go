...
/*
handshake-node is a Handshake full node implementation written in Go, forked from btcd.

The default options are sane for most users.  This means handshake-node will work 'out of
...
parsed when handshake-node starts up.  By default, the configuration file is located at
~/.handshake-node/handshake-node.conf on POSIX-style operating systems and
%LOCALAPPDATA%\Handshake-node\handshake-node.conf on Windows.  The -C (--configfile)
flag, as shown below, can be used to override this location.

Usage:

	handshake-node [OPTIONS]
