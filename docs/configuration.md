...
In addition, if you are starting handshake-node with TLS and want to make it
...
It is a flat, binary file containing blockchain data starting from the
genesis block and continuing through a relatively recent block height depending
on the last time it was updated.

**NOTE:** Using bootstrap.dat is entirely optional.  handshake-node will download the
block chain from other peers through the protocol with no extra
...
1. Stop handshake-node if it is already running.  This is required since addblock needs to
   access the database used by handshake-node and it will be locked if handshake-node is using it.
