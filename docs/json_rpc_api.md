...
The handshake-node RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
handshake-node is `wss://your_ip_or_domain:12037/ws`.
...
to a handshake-node RPC server:

* **rpcuser** is the full-access username configured for the handshake-node RPC server
* **rpcpass** is the full-access password configured for the handshake-node RPC server
* **rpclimituser** is the limited username configured for the handshake-node RPC server
* **rpclimitpass** is the limited password configured for the handshake-node RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the handshake-node
  server is configured with.  It is automatically generated by handshake-node and placed
  in the handshake-node home directory (which is typically `%LOCALAPPDATA%\Handshake-node` on
  Windows and `~/.handshake-node` on POSIX-like OSes)

**NOTE:** As mentioned above, handshake-node is secure by default which means the RPC
...
The handshake-node RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
handshake-node comes with a separate utility named `hnsctl` which can be used to issue
these RPC commands via HTTP POST requests to handshake-node after configuring it with the
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">handshake-node does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since handshake-node does not have the wallet integrated to provide payment addresses, handshake-node must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown handshake-node.|
|29|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|30|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since handshake-node does not have a wallet integrated, handshake-node will only return whether the address is valid or not.|
...
|Notes|<font color="orange">Since handshake-node does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since handshake-node does not have the wallet integrated to provide payment addresses, handshake-node must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">handshake-node does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown handshake-node.|
|Returns|`"handshake-node stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
The following is an overview of the RPC methods which are implemented by handshake-node, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network handshake-node is running on.|None|
...
|Parameters|None|
|Description|Get bitcoin network handshake-node is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of handshake-node.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"hnsjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "x.y.z",  (string) the version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": x,  (numeric) the major version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": y,  (numeric) the minor version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": z,  (numeric) the patch version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",  (string) prerelease info for the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""  (string) metadata about the server build`<br />&nbsp;&nbsp;`}`<br />`}`|
|Example Return|`{`<br />&nbsp;&nbsp;`"hnsjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "1.0.0",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": 1,  `<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""`<br />&nbsp;&nbsp;`}`<br />`}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this handshake-node instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
handshake-node uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll handshake-node for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/blinklabs-io/handshake-node/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/blinklabs-io/handshake-node/tree/master/rpcclient) package to connect with
a handshake-node instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by handshake-node when it starts the RPC server and doesn't already
	// have one.
	hnsNodeHomeDir := hnsutil.AppDataDir("handshake-node", false)
	certs, err := os.ReadFile(filepath.Join(hnsNodeHomeDir, "rpc.cert"))
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[rpcclient](https://github.com/blinklabs-io/handshake-node/tree/master/rpcclient) package to connect with
a handshake-node instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/blinklabs-io/handshake-node/tree/master/rpcclient) package to connect with
a handshake-node instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a handshake-node instance,
...
// generated by handshake-node when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/handshake-node/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The handshake-node generated certificate acts as
// its own certificate authority, so it needs to be specified in the 'ca' array
// for the certificate to properly validate.
var ws = new WebSocket('wss://127.0.0.1:12037/ws', {
  headers: {
