...
func hnsMain(serverChan chan<- *server) error {
	...
	defer hnsLog.Info("Shutdown complete")

	// Show version at startup.
	hnsLog.Infof("Version %s", version())
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			hnsLog.Errorf("Unable to create cpu profile: %v", err)
			return err
		}
	...
	if cfg.MemoryProfile != "" {
		...
		if err != nil {
			hnsLog.Errorf("Unable to create memory profile: %v", err)
			return err
		}
		defer f.Close()
	...
	if cfg.TraceProfile != "" {
		...
		if err != nil {
			hnsLog.Errorf("Unable to create execution trace: %v", err)
			return err
		}
		trace.Start(f)
		defer f.Close()
	...
	// Perform upgrades to handshake-node as new versions require it.
	...
	defer func() {
		...
		hnsLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		hnsLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		hnsLog.Infof("Gracefully shutting down the server...")
		server.Stop()
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		hnsLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		hnsLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		hnsLog.Infof("Creating block database in memory.")
