...
const (
	// svcName is the name of handshake-node service.
	svcName = "handshakenodesvc"
	...
	svcDisplayName = "Handshake Node Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the Handshake block " +
...
// logServiceStartOfDay logs information about handshake-node when the main server has
...
// hnsNodeService houses the main service handler which handles all service
// updates and launching the main application function.
type hnsNodeService struct{}
...
// long-running hnsMain (which is the real meat of handshake-node), handles
// change requests, and notifies the service control manager of changes.
func (s *hnsNodeService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start hnsMain in a separate goroutine so the service can start
...
// installService attempts to install the handshake-node service.  Typically this should
...
// removeService attempts to uninstall the handshake-node service.  Typically this should
...
// startService attempts to start the handshake-node service.
...
func serviceMain() (bool, error) {
	...
	// needed to run handshake-node on Windows in CI environments like Travis.
	...
	err = svc.Run(svcName, &hnsNodeService{})
	if err != nil {
